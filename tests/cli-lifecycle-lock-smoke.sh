...
probe_blocking() {
  local label="$1" command_line="$2" pid
  ( "$TMP/bin/proxy-ctl" $command_line ) >"$TMP/${label}.log" 2>&1 & pid=$!
  sleep 0.15
...
start_pid="$(probe_blocking start start)"
...
stop_pid="$(probe_blocking profile-stop '--profile demo stop')"

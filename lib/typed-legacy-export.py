...
0.5.0 supports one canonical Egress Path shape: direct SSH to a single target. The
legacy ssh-socks target options remain accepted for compatibility. When both forms are
present they must agree; the canonical Egress Path wins only after that consistency check.
"""
...
def validate_direct_egress(config: dict[str, Any], backend: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if path.get("mode") != "direct":
        raise ValueError("egress_path.mode must be direct in 0.5.0")
    ...
    if not isinstance(host, str) or not host:
        raise ValueError("egress_path.target.host must be a non-empty string")
    if any(ch.isspace() for ch in host):
        raise ValueError("egress_path.target.host must not contain whitespace")
    if not isinstance(user, str) or not user:
        raise ValueError("egress_path.target.user must be a non-empty string")
    if any(ch.isspace() for ch in user):
        raise ValueError("egress_path.target.user must not contain whitespace")
    if not isinstance(port, int) or isinstance(port, bool) or not 1 <= port <= 65535:
        raise ValueError("egress_path.target.port must be an integer between 1 and 65535")

    if backend.get("type") != "ssh-socks":
        raise ValueError("egress_path requires backend.type=ssh-socks in 0.5.0")
    ...
    for key, canonical in legacy_map.items():
        if key in options and options[key] not in (None, "") and options[key] != canonical:
            raise ValueError(
                f"egress_path.target.{key.removeprefix('remote_')} conflicts with backend.options.{key}"
            )
    return target
...
def build(config: dict[str, Any]) -> str:
    ...
    if backend["type"] == "ssh-socks":
        mapping = {
            "REMOTE_HOST": target["host"] if target is not None else options.get("remote_host", ""),
            "REMOTE_USER": target["user"] if target is not None else options.get("remote_user", ""),
            "REMOTE_PORT": target["port"] if target is not None else options.get("remote_port", 22),

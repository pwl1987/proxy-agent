...
revision_owner_pid() { printf '%s' "${BASHPID:-$$}"; }
...
revision_lock_reclaim() {
  local lock="$(revision_lock_dir)" guard="$(revision_lock_dir).reclaim" stale="${lock}.stale.$(revision_owner_pid).$RANDOM" guard_created guard_now
...
revision_lock_acquire() {
  revision_init
  local lock="$(revision_lock_dir)" now pid
  for _ in {1..100}; do
...
PY
  ...
  local id="$(revision_next_id)" timestamp file tmp previous; previous="$current"; timestamp="$(date +%s)"; file="$(revision_file "$id")"; tmp="${file}.tmp.$owner"
...
PY
  then rm -f "$tmp"; revision_lock_release; return 1; fi
  mv -f "$tmp" "$file"; printf '%s\n' "$id" >"$(revision_head_file)"
  tmp="$(revision_desired_file).tmp.$owner"; if ! printf '%s\n' "$config_json" >"$tmp"; then rm -f "$tmp"; revision_lock_release; return 1; fi
